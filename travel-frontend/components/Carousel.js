...
export default function Carousel() {
  ...
  return (
    <>
    <div className="absolute flex items-center justify-center w-full px-3 transform -translate-y-1/2 bg-opacity-50 top-1/2 bg-off-white mt-44">
        
    <Image src={featuredPlaces[currentIndex]} alt="Places to visit" ref={carouselRef} className="justify-center rounded-md cursor-pointer sm:h-2/3 sm:w-4/5 sm:aspect-video lg:h-1/5 lg:w-5/12 lg:mx-auto lg:mb-8 hover:opacity-70" />
    <div className="absolute flex items-center justify-between w-full px-3 transform -translate-y-1/2 top-1/2">

    <img src='/left-arrow.png' onClick={handleOnClickPrev}  className= "h-12 pl-1 transition cursor-pointer opacity-80 hover:opacity-50"/>
    <img src='/right-arrow.png' onClick={handleOnClickNext} className= "h-12 pl-1 transition cursor-pointer opacity-80 hover:opacity-50" />
